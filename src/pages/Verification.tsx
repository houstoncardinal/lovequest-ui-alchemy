  ...
  const loadVerificationData = async () => {
    ...
    try {
      ...
      const verificationsQuery = query(
        ...
        where('verificationType', '==', 'identity'),
      ...
      if (!querySnapshot.empty) {
        ...
        setVerificationRequest({ id: verificationDoc.id, ...verificationDoc.data() } as VerificationRequest);
      } else {
      ...
      if (userProfile) {
        setProfile({
          canAccessApp: userProfile.canAccessApp ?? false,
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className={`font-medium ${
                    profile?.canAccessApp ? 'text-green-800' : 'text-amber-800'
                  }`}>
                    {profile?.canAccessApp ? 'Full Access Granted' : 'Limited Access'}
                  </p>
                  <p className={`text-sm ${
                    profile?.canAccessApp ? 'text-green-700' : 'text-amber-700'
                  }`}>
                    {profile?.canAccessApp
                ...
                {verificationRequest && (
                  <div className="bg-white rounded border p-3">
                    <p className="text-xs text-amber-600">
                      Submitted on {new Date(verificationRequest.createdAt).toLocaleDateString()} at {new Date(verificationRequest.createdAt).toLocaleTimeString()}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-red-50 border border-red-200 rounded-lg p-6">
                ...
                {verificationRequest?.adminNotes && (
                  <div className="bg-white rounded border p-4 mb-4">
                    ...
                    <p className="text-sm text-red-800">{verificationRequest.adminNotes}</p>
                  </div>
                )}

...
const RelationshipFeatures = () => {
  ...
  const fetchEmergencyContact = async () => {
    ...
    const stored = localStorage.getItem(`emergency_contact_${user?.id}`);
    if (stored) {
  ...
  const saveEmergencyContact = async () => {
    try {
      ...
      localStorage.setItem(`emergency_contact_${user?.id}`, JSON.stringify(emergencyContact));
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      id="notify"
                      checked={emergencyContact.notify_on_dates}
                      onCheckedChange={(checked) => setEmergencyContact(prev => ({ ...prev, notify_on_dates: checked }))}
                    />

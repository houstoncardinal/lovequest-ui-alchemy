...
import { db, collection, query, where, getDocs, updateDoc, doc, orderBy as firestoreOrderBy } from '@/integrations/firebase';
...
const QuickReview: React.FC<QuickReviewProps> = ({ onBack }) => {
  ...
  const handleVerificationAction = async (action: 'approve' | 'reject') => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'verificationRequests', currentRequest.id), {
        status,
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center gap-4">
                <Avatar className="w-16 h-16">
                  <AvatarImage src={currentRequest.profiles?.photoURL} />
                  <AvatarFallback>
                    {currentRequest.profiles?.firstName?.[0]}{currentRequest.profiles?.lastName?.[0]}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <h3 className="font-semibold">{currentRequest.profiles?.displayName}</h3>
              ...
              <div className="pt-4 border-t">
                ...
                <p className="text-sm text-gray-600">
                  {new Date(currentRequest.createdAt).toLocaleDateString()} at{' '}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {currentRequest.idDocumentUrl && (
                  <div>
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <img
                        src={currentRequest.idDocumentUrl}
                        alt="ID Document"
                ...
                {currentRequest.facePhotoUrl && (
                  <div>
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <img
                        src={currentRequest.facePhotoUrl}
                        alt="Face Photo"
                        className="w-full h-64 object-contain bg-gray-50"
                      />
                    </div>

...
const PostCard = ({ post, currentUserId, onLike, onEdit, onDelete, formatTimeAgo }: PostCardProps) => {
  ...
  const displayName = post.profiles?.displayName ||
    `${post.profiles?.firstName} ${post.profiles?.lastName}`.trim() ||
    'Unknown User';

  const initials = post.profiles?.firstName?.[0] + (post.profiles?.lastName?.[0] || '');
            ...
            <div className="flex items-center gap-3">
              <div className="relative group">
                <Avatar className="ring-2 ring-primary/20 ring-offset-2 ring-offset-background transition-all duration-300 group-hover:ring-primary/40">
                  <AvatarImage src={post.profiles?.photoURL} className="object-cover" />

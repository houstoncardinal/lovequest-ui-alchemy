-- Fix the ambiguous gender column reference in the function
CREATE OR REPLACE FUNCTION public.get_enhanced_match_recommendations_with_gender(target_user_id uuid, limit_count integer DEFAULT 10)
 RETURNS TABLE(user_id uuid, first_name text, last_name text, display_name text, age integer, gender text, location text, bio text, avatar_url text, religion_level text, prayer_frequency text, hijab_status text, education_level text, career_field text, marital_status text, smoking_status text, has_children boolean, children_preference text, is_verified boolean, interests text[], match_score integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Get age preferences if they exist
  SELECT COALESCE(up.age_range_min, 18), COALESCE(up.age_range_max, 65) 
  INTO target_age_min, target_age_max
  ...
  WHERE p.user_id != target_user_id
    ...
    -- Age filtering
    AND (p.age IS NULL OR (p.age >= target_age_min AND p.age <= target_age_max))
